...
alleEinige <- function(zahlen, wert) c(all(zahlen > wert), any(zahlen > wert)) # Argumentnamen frei änderbar
...
anzahlAnteil <- function(zahlen, wert) {
    anzahl <- length(which(zahlen > wert))
    c(anzahl, anzahl / length(zahlen))
}
...
zweitesMaximum <- function(x) max(x[-(which(x == max(x))[1])])
